...
class DataReporter:
    ...
    def albums(self, df: pd.DataFrame):
        ...
        logger.info(f"Albums loaded {self.num_album.success}")
        ...
        logger.info(f"Albums not loaded {self.num_album.failure}")

        albums_failed = HISTORY_PATH / "albums_failed_detailed.csv"
        df_failed = df[fail_mask]
        df_failed.to_csv(albums_failed)
        logger.info(f"Albums not loaded written to {albums_failed}")
        ...
        logger.info(f"Albums not loaded summary written to {albums_summary}")
    ...
    def playlists(df_playlist: pd.DataFrame):
        ...
        logger.info(f"Playlist tracks not loaded written to {playlists_failed}")
    ...
    def tracks(df):
        ...
        logger.info(f"Tracks not loaded written to {tracks_failed}")
